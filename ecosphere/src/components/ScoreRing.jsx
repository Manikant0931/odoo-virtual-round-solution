const RING_COLORS = { environmental: "#356B4E", social: "#C55E4A", governance: "#4F63B8" };
...
function Arc({ cx, cy, r, pct, color, width }) {
  ...
  return (
    <g transform={`rotate(-90 ${cx} ${cy})`}>
      <circle cx={cx} cy={cy} r={r} fill="none" stroke={color} strokeOpacity="0.16" strokeWidth={width} />
      <circle
        cx={cx}
        cy={cy}
        r={r}
  ...
  return (
    <div className="flex items-center gap-5">
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} role="img" aria-label={`${label}: ${overall} out of 100`}>
        ...
        <text x={cx} y={cy - 4} textAnchor="middle" className="fill-ink-950" style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, fontSize: size * 0.2 }}>
          {overall}
        </text>
        <text x={cx} y={cy + size * 0.11} textAnchor="middle" className="fill-ink-700" style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: size * 0.055 }}>
          / 100
        </text>
      </svg>
